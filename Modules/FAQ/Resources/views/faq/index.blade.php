    ...
    <div class="box-content">
        <div class="table-list-content">
            @if(in_array('faqs.add',isset($perms['faqs'])?$perms['faqs']:[]) || $super == 1)
                <a href="{{route('admin.faqs.add')}}" class="btn btn-primary"><i class="bx bx-plus me-sm-2"></i>Tạo Faqs</a>
            @endif
...
{{--                            <th>answer</th>--}}
                        ...
                        @if (count($faqs))
                            @foreach ($faqs as $key =>$item)
                                <tr>
                                    <td>
                                        @if(in_array('faqs.edit',isset($perms['Faqs'])?$perms['Faqs']:[]) || $super == 1)
                                            <a href="{{route('admin.faqs.edit',['id'=>$item->id])}}">{{$item->question}}</a>
                                        @else
                                            {{$item->question}}
                                        @endif
                                    </td>
{{--                                    <td>{{$item->answer}}</td>--}}
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                @endif
                                                @if(in_array('faqs.delete',isset($perms['Faqs'])?$perms['Faqs']:[]) || $super == 1)
                                                    <a class="dropdown-item"
                                                       href="{{route('admin.faqs.delete',['id'=>$item->id])}}"><i
                    ...
                    <div class="row mbm">
                        <div class="col-sm-3">
                            <span class="record-total">Tổng: {{ $faqs->total() }} bản ghi</span>
                        </div>
                        <div class="col-sm-6 text-center">
                            <div class="pagination-panel">
                                {{ $faqs->appends(Request::all())->onEachSide(1)->links('pagination::bootstrap-4') }}
                            </div>
                        </div>

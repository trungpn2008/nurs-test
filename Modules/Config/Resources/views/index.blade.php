...
@section('content')
    ...
    <div class="box-content">
        <div class="table-list-content">
            <a href="{{route('admin.config.add')}}" class="btn btn-primary"><i class="bx bx-plus me-sm-2"></i>Tạo</a>
            ...
            <div class="table-responsive text-nowrap">
                <form action="{{route('admin.action')}}" method="post" id="form-index-action">
                        ...
                        @if (count($config))
                            @foreach ($config as $key =>$item)
                                <tr>
                                    ...
                                    <td><a href="{{route('admin.config.edit',['id'=>$item->id])}}">{{$item->title}}</a>
                                    </td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item"
                                                   href="{{route('admin.config.edit',['id'=>$item->id])}}"><i
                                                        class="bx bx-edit-alt me-1"></i> Edit</a>
                                                <a class="dropdown-item"
                                                   href="{{route('admin.config.delete',['id'=>$item->id])}}"><i
                    ...
                    <div class="row mbm">
                        <div class="col-sm-3">
                            <span class="record-total">Tổng: {{ $config->total() }} bản ghi</span>
                        </div>
                        <div class="col-sm-6 text-center">
                            <div class="pagination-panel">
                                {{ $config->appends(Request::all())->onEachSide(1)->links('pagination::bootstrap-4') }}
                            </div>
                        </div>

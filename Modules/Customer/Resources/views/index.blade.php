                    ...
                    <table class="table table-hover">
                        ...
                        @if (count($customers))
                            @foreach ($customers as $key =>$item)
                                <tr>
                                    <td>
                                        @if(in_array('customer.show',isset($perms['Customer'])?$perms['Customer']:[]) || $super == 1)
                                            <a href="{{route('admin.customer.show',['id'=>$item->id])}}">{{$item->name}}</a>
                                        @else
                                            {{$item->name}}
                                        @endif
                                    </td>
                                            ...
                                            <div class="dropdown-menu">
                                                @if(in_array('customer.show',isset($perms['Customer'])?$perms['Customer']:[]) || $super == 1)
                                                    <a class="dropdown-item"
                                                       href="{{route('admin.customer.show',['id'=>$item->id])}}"><i
                    ...
                    <div class="row mbm">
                        ...
                        <div class="col-sm-6 text-center">
                            <div class="pagination-panel">
                                {{ $customers->appends(Request::all())->onEachSide(1)->links('pagination::bootstrap-4') }}
                            </div>
                        </div>

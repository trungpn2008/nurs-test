...
@section('content')
    <h4 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light">QA Cate /</span> List
    </h4>
    <div class="box-content">
        <div class="table-list-content">
            @if(in_array('qa-cate.add',isset($perms['QACate'])?$perms['QACate']:[]) || $super == 1)
                <a href="{{route('admin.qa-cate.add')}}" class="btn btn-primary"><i class="bx bx-plus me-sm-2"></i>Tạo QA Cate</a>
            @endif
            ...
            <div class="table-responsive text-nowrap">
                <form action="{{route('admin.action')}}" method="post" id="form-index-action">
                    ...
                    <table class="table table-hover">
                        ...
                        @if (count($qaCate))
                            @foreach ($qaCate as $key =>$item)
                                <tr>
                                    <td>
                                        @if(in_array('qa-cate.edit',isset($perms['QACate'])?$perms['QACate']:[]) || $super == 1)
                                            <a href="{{route('admin.qa-cate.edit',['id'=>$item->id])}}">{{$item->title}}</a>
                                        @else
                                            {{$item->title}}
                                        @endif
                                    </td>
                                            ...
                                            <div class="dropdown-menu">
                                                @if(in_array('qa-cate.edit',isset($perms['QACate'])?$perms['QACate']:[]) || $super == 1)
                                                    <a class="dropdown-item"
                                                       href="{{route('admin.qa-cate.edit',['id'=>$item->id])}}"><i
                                                            class="bx bx-edit-alt me-1"></i> Edit</a>
                                                @endif
                                                @if(in_array('qa-cate.delete',isset($perms['QACate'])?$perms['QACate']:[]) || $super == 1)
                                                    <a class="dropdown-item"
                                                       href="{{route('admin.qa-cate.delete',['id'=>$item->id])}}"><i
                    ...
                    <div class="row mbm">
                        <div class="col-sm-3">
                            <span class="record-total">Tổng: {{ $qaCate->total() }} bản ghi</span>
                        </div>
                        <div class="col-sm-6 text-center">
                            <div class="pagination-panel">
                                {{ $qaCate->appends(Request::all())->onEachSide(1)->links('pagination::bootstrap-4') }}
                            </div>
                        </div>

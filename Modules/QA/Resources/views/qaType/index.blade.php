...
@section('content')
    <h4 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light">QA Type /</span> List
    </h4>
    <div class="box-content">
        <div class="table-list-content">
            @if(in_array('qa-type.add',isset($perms['QAType'])?$perms['QAType']:[]) || $super == 1)
                <a href="{{route('admin.qa-type.add')}}" class="btn btn-primary"><i class="bx bx-plus me-sm-2"></i>Tạo QA Type</a>
            @endif
            ...
            <div class="table-responsive text-nowrap">
                <form action="{{route('admin.action')}}" method="post" id="form-index-action">
                    ...
                    <table class="table table-hover">
                        ...
                        @if (count($qaType))
                            @foreach ($qaType as $key =>$item)
                                <tr>
                                    <td>
                                        @if(in_array('qa-type.edit',isset($perms['QAType'])?$perms['QAType']:[]) || $super == 1)
                                            <a href="{{route('admin.qa-type.edit',['id'=>$item->id])}}">{{$item->title}}</a>
                                        @else
                                            {{$item->title}}
                                        @endif
                                    </td>
                                            ...
                                            <div class="dropdown-menu">
                                                @if(in_array('qa-type.edit',isset($perms['QAType'])?$perms['QAType']:[]) || $super == 1)
                                                    <a class="dropdown-item"
                                                       href="{{route('admin.qa-type.edit',['id'=>$item->id])}}"><i
                                                            class="bx bx-edit-alt me-1"></i> Edit</a>
                                                @endif
                                                @if(in_array('qa-type.delete',isset($perms['QAType'])?$perms['QAType']:[]) || $super == 1)
                                                    <a class="dropdown-item"
                                                       href="{{route('admin.qa-type.delete',['id'=>$item->id])}}"><i
                    ...
                    <div class="row mbm">
                        <div class="col-sm-3">
                            <span class="record-total">Tổng: {{ $qaType->total() }} bản ghi</span>
                        </div>
                        <div class="col-sm-6 text-center">
                            <div class="pagination-panel">
                                {{ $qaType->appends(Request::all())->onEachSide(1)->links('pagination::bootstrap-4') }}
                            </div>
                        </div>
